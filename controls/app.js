...
app.use(
  session({ secret: 'thisissecret', resave: false, saveUninitialized: true })
);
...
app.get('/index', function (req, res) {
  if (req.session.theUser == undefined) {
    ...
    res.render('index', {
      session: [1],
      Userfname: req.session.theUser.firstName,
    });
  }
});
...
app.get(
  '/items?:itemurl',
  check('itemcode', 'invalid itemcode').isAlphanumeric().trim(),
  function (req, res) {
    var errors = validationResult(req);
    if (!errors.isEmpty()) {
      //checks for errors if errors no item inforation will be shown and sent to ejs pages
      ...
      //executes when there are no errors
      var item_code = req.query.itemcode; //itemcode in itemurl

      if (req.session.theUser == undefined) {
        //when user is not logged in
        item.find({ itemcode: item_code }, function (err, itemdata) {
          //query to get itemdetails
          if (err) throw err;
          ...
          specificuserrating(itemdata[0].UserId)
        ...
        var query = {
          $and: [
            /*query to check if the item is in offer table */
            {
              $or: [{ status: 'pending' }, { status: 'swapped' }],
            } /*query is done on offers db to use the offer details for getting offer feedback */,
            {
              $or: [{ offererId: UserId }, { offereeId: UserId }],
            } /*And also the query helps in showing the swap avilable button only for the users involved in the swap*/,
            {
        ...
        offer
          ...
          .then(function (offerdata) {
            if (offerdata.length != 0) {
              item
                .find({ itemcode: item_code })
                .then(function (item) {
                  specificuserrating(item[0].UserId) //getuser rating to show in the ejs page
                    .then(function (avgval) {
                      if (
                        item[0].UserId == UserId ||
                        offerdata[0].status == 'swapped'
                      ) {
                        //condition for item rating by user who own it or the one who swapped it
                        itemFeedback
                          .find({ UserId: UserId, itemcode: item_code }) //query to send itemrating to ejs page
                          .then(function (itemfeedback) {
                            ...
                            if (
                              ...
                              //condition to rate the swapper who involved in the swap
                              var rateswapper = [1];
                              userFeedback
                                .find({
                                  offerId: offerdata[0].offerId,
                                  UserId1: UserId,
                                  UserId2: item[0].UserId,
                                }) //query to get the ratings between two users for specific offer
                                .then(function (userfeedbackdata) {
                                  ...
                                  if (offerdata[0].offererId == UserId) {
                                    //sending hidden parameters to use for adduserfeedback
              ...
              //not in offer collection so can participate in swap
              item
                .find({ itemcode: item_code })
                .then(function (item) {
                  if (item.length != 0) {
                    specificuserrating(item[0].UserId) //gets user rating
                      .then(function (avgval) {
                        if (item[0].UserId == req.session.theUser.UserId) {
                          itemFeedback
                            .find({ UserId: UserId, itemcode: item_code })
                            .then(function (itemfeedback) {
