...
var itemsSchema=new Schema({
  ...
  versionKey: false,collection:'items' , toObject: { virtuals: true } 
});
...
itemsSchema.statics.getotherthanuseritems=function(UserId,catalogcategory,cb){
  if(!catalogcategory){
    return this.find({UserId:{$ne:UserId},itemstatus:{$ne:'swapped'}},cb);
  }
  else{
    return this.find({UserId:{$ne:UserId},itemstatus:{$ne:'swapped'},catalogcategory:catalogcategory},cb);
  }
...
itemsSchema.statics.getitemsbycategory=function(catalogcategory,cb){
  if(!catalogcategory){
    ...
    return this.find({catalogcategory:catalogcategory,itemstatus:{$ne:'swapped'}},cb);
  }
...
itemsSchema.statics.updateItemstatus=function(itemcode,status,cb){
  return this.findOneAndUpdate({itemcode:itemcode},{itemstatus:status},cb)
}
var item=mongoose.model('item',itemsSchema);

/**add new item method */
var additem=function(itemcode,UserId, itemname,itemsname, catalogcategory, itemby, description,detaildescription, imgurl){
var Newitem={itemcode:itemcode,
  UserId:UserId,
  itemname:itemname,
  itemsname:itemsname,
    ...
    if(user.length!=0){
      newitem.save(function(err){
        if(err) 
        {
         ...
         console.log('item with item code :'+Item.itemcode+' is saved to data base');
        }
          
      })
    }
    else{
      console.log('UserId doesnot exists so newitem cannot be created with new userID')
    }
  })
