...
var offersSchema=new Schema({
    ...
    versionKey: false,collection:'offers',toObject: { virtuals: true } 
})
            ...
            offer.find({offerId:offerId})
                .then(function(offerdetails){
                    ...
                    if(status=='Rejected'||status=='withdrawn'){
                        ...
                        offer.find({$or:[{offereeitemId:offerdetails[0].offereeitemId},{offereritemId:offerdetails[0].offereeitemId}
                        ,{offereeitemId:offerdetails[0].offereritemId},{offereritemId:offerdetails[0].offereritemId}],status:'pending'},function(err,data){
                            if(err) throw err;
                            if(data.length==1){
                        ...
                        var query={$or:[{itemcode:offerdetails[0].offereritemId},{itemcode:offerdetails[0].offereeitemId}]}
                    ...
                    /*offer update*/
                    offer.findOneAndUpdate({offerId:offerId},offerstatus,{new:true},function(err,updatedoffer){
                        if(err){
                            reject(err);
                        } 
                        else{
                            if(updatedoffer.status=='swapped'){
                                ...
                                offer.find({$or:[{offereeitemId:updatedoffer.offereeitemId},{offereritemId:updatedoffer.offereeitemId}],status:'pending'},function(err,offersarray){
                                    if(offersarray.length!=0){
                                        for(var i=0;i<offersarray.length;i++){
                                            offer.findOneAndUpdate({offerId:offersarray[i].offerId},{status:'already swapped by user/ better offer found'},{new:true},function(err,updated){
                                                if(err) throw err;
                                                console.log(updated.offerId+': '+updated.status)
                                            })
                                             ...
                                             */
                                            if(offersarray[i].offereeitemId==updatedoffer.offereeitemId){
                                                item.findOneAndUpdate({itemcode:offersarray[i].offereritemId},{itemstatus:'available'},{new:true},function(err,updateditem){
                                                    if(err) throw err;
                                                    console.log('updateditem:'+updateditem.itemname)
                                                })
                                            }
                                             ...
                                             */
                                            else if(offersarray[i].offereritemId==updatedoffer.offereeitemId){
                                                item.findOneAndUpdate({itemcode:offersarray[i].offereeitemId},{itemstatus:'available'},{new:true},function(err,updateditem){
                                                    if(err) throw err;
                                                    console.log('updateditem:'+updateditem.itemname)
                                                })
                                            }
...
var Addoffer=function(Newoffer){
    offercounter.findByIdAndUpdate({_id: 'offerCounter'}, {$inc: { seq: 1} }, {new: true, upsert: true}).then(function(count) {
    ...
    newoffer.save(function(err,res){
        if (err) {
            ...
            var query={$or:[{itemcode:newoffer.offereritemId},{itemcode:newoffer.offereeitemId}]}
            var update={'itemstatus':'pending'}
            item.update(query,update,{'multi':true},function(err,res){
                if(err) throw err;
                
            });
            console.log('newoffer with offerID: '+res.offerId+' and offererId: '+res.offererId+' is saved')
...
var removeOffer=function(UserId,itemcode){
    return new Promise(function(resolve,reject){
        var query={$or:[{offererId:UserId},{offereeId:UserId}],
...
var useroffers=function(UserId){
    return new Promise(function(resolve,reject){
        offer.find({$or:[{offererId:UserId},{offereeId:UserId}]}).populate('offereritem','itemstatus itemsname').populate('offereeitem','itemstatus itemsname')
